...
public class MallOrderStarAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("MallOrderStarAction----execute()____");
		
		request.setCharacterEncoding("UTF-8");
		
		//세션값 체크
		HttpSession session = request.getSession();
		//String check = (String)session.getAttribute(체크할 값);
		ActionForward forward = new ActionForward();
		//if(session==null){	//체크값 비교
		...
		// 현재 관리자의 내용만 가져옴
		//admin을 세션값의 id를 가져옴 or 세션값이 id가 아닐경우 수정 필요
		MemberDTO memdto = memdao.getMember("admin"); 
